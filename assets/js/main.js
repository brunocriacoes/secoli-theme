...
async function buscarDadosAPI(urlBase, parametros) {
  function construirParametrosConsulta(params) {
    return Object.keys(params)
      .map(
        (chave) =>
          encodeURIComponent(chave) + "=" + encodeURIComponent(params[chave])
      )
      .join("&");
  }
...
async function addLead(event, $e) {
  ...
  let addLeadApi = await buscarDadosAPI(
    _domain + "/wp-json/secoli/v1/lead/",
    payload
  );
...
async function addContact(event, $e) {
  ...
  let alert = document.querySelector(".news__alert");
  let message = document.querySelector(".news__alert span");
  let addContactApi = await buscarDadosAPI(
    _domain + "/wp-json/secoli/v1/agendar-contato/",
    payload
  );
...
$(document).ready(function () {
  // Quando um link de categoria é clicado
  $(".menu_category__link_mobile").click(function (e) {
    e.preventDefault(); // Previne o comportamento padrão do link

    // Encontra o próximo elemento <ul> (que contém as subcategorias) e alterna a visibilidade
    $(this).next("ul").slideToggle("fast");

    // Alterna a classe do ícone de seta para cima/para baixo (se desejar)
    $(this)
