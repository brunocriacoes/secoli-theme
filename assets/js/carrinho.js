class Cart {
  ...
  tpl_quantity(payload, i, id) {
    return `
        <label class="item__in__cart__input">
            <b>QTA-${i + 1}</b>
            <input type="number" oninput="cart.atualizarQuantidade('${id}', '${i}', this.value)" name="quant[${id}][]"  min="60" value="${payload.value}" required>
        </label>
        `;
  }

  tpl_item(payload, i) {
    let values = payload.quantity
      .map((p, i) => this.tpl_quantity(p, i, payload.id))
      .join("");
    return `
  ...
  render() {
    ...
    if ($loop_itens) {
      $loop_itens.innerHTML = this.products
        .map((data) => this.tpl_item(data))
        .join(" ");
    }
  }
}
